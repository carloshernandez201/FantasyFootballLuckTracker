...
class BaseOAuth(object):
    ...
    def handler(self,):
        ...
        if self.browser_callback:
            ...
            # Use an environment variable or a programmatic override for the verifier
            self.verifier = os.getenv("YFPY_VERIFIER")

        # Wait for up to 10 seconds for the verifier to be set
        if not self.verifier:
            print("Waiting for verifier to be set in environment...")
            for _ in range(20):  # Retry every second for up to 10 seconds
                time.sleep(1)
                print(_)
                self.verifier = os.getenv("YFPY_VERIFIER")
                if self.verifier:
        ...
        # If still not set, fall back to manual input
        if not self.verifier:
            self.verifier = input("Edsacid nter verifier : ")
